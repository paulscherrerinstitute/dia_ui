...
Detector Integration API User Interface project is a webserver to control, debug and visualize informations related to the Detector Integration API (DIA status, Backend configuration, Writer configuration and Detector configuration).
...
- DIA User Interface: http://xbl-daq-29:5010
...
- The DIA UI is served on address: **http://xbl-daq-29:5010**

DIA UI is run using a **systemd** service (/etc/systemd/system/dia_ui.service). The services invokes the startup file **/home/dia_ui/start_dia_ui.sh**.
...
and open the address on any browser. Default is **https://0.0.0.0:5010**.

Package is located in https://anaconda.org/paulscherrerinstitute/dia_ui
...
> **Note**: Default values for `<HOST>`, `<PORT>` are 0.0.0.0 and 5010.
...
A script is provided to set a debug configuration into dia debug server:

    $ python set_debug_server_config.py 

> It basically verifies if the DIA server is up and running, and sets a initial configuration (assuming it runs on http://0.0.0.0:10000), displayed below:
...
## ZMQ statistics configuration
The sender (writer, backend or detector) needs to configure the ZMQ bind as follows:

    context = zmq.Context()
    socketPub = context.socket(zmq.PUB)
    socketPub.bind("tcp://*:8088")

A filter is used to define to which sender the message belongs:

- Writer: ```statisticsWriter```
- Backend: ```statisticsBackend```
- Detector: ```statisticsDetector```

To send a zmq statistics json package:

    # filter definitions
    topicWriter = 'statisticsWriter'
    # writerStartJson definition
    writerStartJson = {"statistics_wr_start": {
            "first_frame_id": randint(0, 9),
            ...
            ...
        }
    # Sends the zmq package
...
## Sending backend statistics:
TODO

## Sending detector statistics:
TODO
